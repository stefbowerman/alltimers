...
if (variant.featured_image) {
  var temp = variant.featured_image.src;
  if (temp.includes(".jpg")) {
    var newImg = temp.replace(".jpg", "_large.jpg");
  } else if (temp.includes(".png")) {
    var newImg = temp.replace(".png", "_large.png");
  }
...
theme.variables = {
  ...
  mediaQueryMedium    : 'screen and (min-width: 769px) and (max-width: 1024px)',
  mediaQueryLarge     : 'screen and (min-width: 1025px)',
  bpSmall             : false
};
...
$(theme.init);
            ...
            $('.add_to_cart').on('click',function(e){
              e.preventDefault();
              var main_product_id = $('.product-single__variants option:selected').data('id');
              var secondary_product_id = $('.extra-product-dropdown option:selected').data('id');
              //               console.log('------', main_product_id , secondary_product_id);

              var params = {
                type: 'POST',
                url: '/cart/add.js',
                data: jQuery('.product-single__form').serialize(),
                dataType: 'json',
              ...
              setTimeout(function(){
                if(secondary_product_id != ""){
                  $.ajax({
                    type: 'POST',
                    url: '/cart/add.js',
                    data: 'quantity='+1+'&id='+secondary_product_id,
                    dataType: 'json',
                    async:false,
                    success: function() {
                      console.log('secondary product added')
